...
def step(bodies: list[Body], field_: TerrainField, dt: float) -> float:
    """Bitta fizika qadami. Qaytaradi: shu qadamdagi maksimal penetratsiya."""
    max_pen = 0.0
    for b in bodies:
        if b.resting:
            continue
        # Tortishish + havo qarshiligi.
        b.vy -= GRAVITY * dt
...
def simulate(bodies: list[Body], field_: TerrainField, *, dt: float = 1.0 / 120.0,
             max_steps: int = 2000, track: int = 0) -> SimStats:
    """Barcha jismlar cho'kkuncha yoki max_steps gacha simulyatsiya."""
    stats = SimStats()
    for s in range(max_steps):
        pen = step(bodies, field_, dt)
